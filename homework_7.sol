...
contract BirthdayPayout {
    ...
    function checkLastPayout(uint256 index) public view returns (bool) {
        uint256 last_payout = getTeammate(index).last_payout;
        (
            uint256 last_payout_year,
            uint256 last_payout_month,
            uint256 last_payout_day
        ) = getDate(last_payout);
        uint256 today = block.timestamp;
        (uint256 today_year, uint256 today_month, uint256 today_day) = getDate(
            today
        );

        if (
            last_payout_day == today_day &&
            last_payout_month == today_month &&
            last_payout_year == today_year
        ) {
            return false;
        }
        return true;
